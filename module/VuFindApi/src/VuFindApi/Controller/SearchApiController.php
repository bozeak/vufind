    ...
    implements ApiInterface
{
    ...
    {
        $eventInstance = $this->getEvent();
        $routeMatch = $eventInstance->getRouteMatch();
        if (isset($routeMatch)) {
            $consumer = $routeMatch->getParam('consumer');
        }

        if (empty($consumer)) {
            return $this->output([], self::STATUS_ERROR, 400, 'Consumer not provided.');
        }
        ...
        $hiddenFilters[] = 'institution:' . $consumer;
